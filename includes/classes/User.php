...
class User {
  ...
  public function getMutualFriends($username_to_check) {
    // friend array for user logged in
    $mutualFriends = 0;
    $user_array = $this->user['friend_array'];
    $user_array_explode = explode(",", $user_array);

    // friend array for user to check
    $query = mysqli_query($this->conn, "SELECT friend_array FROM users WHERE username = '$username_to_check'");
    $row = mysqli_fetch_array($query);
    ...
    // holds total value of friends in common
